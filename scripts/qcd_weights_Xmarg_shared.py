    ...
    for _ in range(N):
        ...
        for i, (e, p, n) in enumerate(zip(e_list, p_list, n_list)):
            if np.isnan(p) or np.isnan(e):
                ...
                weights[i] += int(pqcd.maximised_likelihood(e0=e,p0=p,n0=n,X=np.exp(logX)))
...
fixed_pressures = np.array([
    np.loadtxt(f'../data/quantities_at_fixed_n/pressure_{n:02}nsat.dat') 
    for n in fixed_matching_densities
    ])

fixed_energy_densities = np.array([
    np.loadtxt(f'../data/quantities_at_fixed_n/energy_density_{n:02}nsat.dat') 
    for n in fixed_matching_densities
    ])

# Load the nTOV quantities
ntov_pressure = np.loadtxt('../data/quantities_at_ntov/pressure.dat')
ntov_energy_density = np.loadtxt('../data/quantities_at_ntov/energy_density.dat')
...
for i, n in enumerate(fixed_matching_densities):
    np.savetxt(f'../data/weights_Xmarg_shared/qcd_weights_{n:02}nsat_Xmarg_shared.dat', qcd_weights[:,i])

# Save the nTOV weights to disk
np.savetxt('../data/weights_Xmarg_shared/qcd_weights_ntov_Xmarg_shared.dat', qcd_weights[:,-1])