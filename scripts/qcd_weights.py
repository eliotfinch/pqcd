    ...
    for e, p in zip(energy_density, pressure):
        for X in X_list:
            if np.isnan(e) or np.isnan(p):
                ...
                qcd_weights[nterm][X].append(pqcd.maximised_likelihood(e0=e, p0=p, n0=nterm*nsat, X=X))

# Save the weights to disk
for nterm in nterm_list:
    for X in X_list:
        np.savetxt(f'../data/weights/qcd_weights_{nterm:02}nsat_X{X}.dat', qcd_weights[nterm][X])
...
for e, p, ntov in zip(energy_density_tov, pressure_tov, collated_ntov):
    for X in X_list:
        qcd_weights_ntov[X].append(pqcd.maximised_likelihood(e0=e, p0=p, n0=ntov, X=X))

for X in X_list:
    np.savetxt(f'../data/weights/qcd_weights_ntov_X{X}.dat', qcd_weights_ntov[X])