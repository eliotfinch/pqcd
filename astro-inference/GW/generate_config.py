...
def generate_config(
    ...
    config['Marginalization'] = {
        'prior': 'default',
        'chirp-mass-range': str((0.99*in_data.loc["mc", "lb"], 1.01*in_data.loc["mc", "ub"])),
        'mass-ratio-range': str((0.99*in_data.loc["q", "lb"], min(1.01*in_data.loc["q", "ub"], 1.0)))
        }
    ...
    config['Submission'] = {
        ...
        'condor-kwargs': {'request_disk': '10 MB', 'request_memory': '2048 MB', 'universe': 'vanilla'}
        }
...
def generate_170817_and_190425_inis(eos_tag, eos_directory, eos_indices_file, eos_per_dir=1000):
...
if __name__ == "__main__":
    ...
    eos_tag = 'margagn'
    eos_per_dir = 1000
    eos_directory = f'/home/eliot.finch/eos/pqcd/data/eos-draws-modified/{set_number}/{eos_tag}'
    eos_indices_file = f'/home/eliot.finch/eos/pqcd/data/eos-draws-modified/{set_number}/{eos_tag}-manifest.csv'

    if not os.path.exists(eos_tag):
      os.mkdir(eos_tag)

    generate_170817_and_190425_inis(eos_tag, eos_directory, eos_indices_file, eos_per_dir)
