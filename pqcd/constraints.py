...
from .pqcd import epsilon_func, pressure_integral
...
class constraints:
    ...
    def nmin_scalar(self, mu):
        ...
        if self.muL <= mu <= self.muc:
            ...
            numerator = mu**3*self.nH - \
                mu*self.muH*(self.muH*self.nH - 2*self.Deltap)
    ...
    def nmax_scalar(self, mu):
        ...
        if self.muL <= mu < self.muc:
            numerator = mu**3*self.nL - \
                mu*self.muL*(self.muL*self.nL + 2*self.Deltap)
    ...
    def plot_mu_n(self, mu_array):
        ...
        ax.plot(
            ...
            self.nmin(mu_array),
            c='C0',
            label=r'$n_\mathrm{min}$'
            )
        ax.plot(
            mu_array,
            self.nmax(mu_array),
            c='C1',
            label=r'$n_\mathrm{max}$'
            )
        ...
        ax.plot(
            ...
            'o',
            c='C4',
            label=r'$(\mu_\mathrm{L}, n_\mathrm{L})$'
            )
        ax.plot(
            self.muH,
            self.nH,
            'o',
            c='C5',
            label=r'$(\mu_\mathrm{H}, n_\mathrm{H})$'
            )
    ...
    def plot_epsilon_p(self, mu_array, log=True):
        ...
        ax.plot(
            epsilon_func(
                mu_array,
                self.nc(mu_array),
                self.pmax(mu_array, self.nc(mu_array))
                ),
        ...
        ax.plot(
            ...
            'o',
            c='C4',
            label=r'$(\epsilon_\mathrm{L}, p_\mathrm{L})$'
            )
        ax.plot(
            self.epsilonH,
            self.pH,
            'o',
            c='C5',
            label=r'$(\epsilon_\mathrm{H}, p_\mathrm{H})$'
            )
